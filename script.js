const students = [
  ...
  //rendering user info into the table
  renderStudents();

  //whenever form submit that time input data will store into student array
  studentForm.addEventListener("submit", function (event) {
    //By default clicking the submit button of form element refresh the page , to prevent this we need to use preventDefault() method
    ...
    const age = parseInt(ageInput.value); //by default input value in String format
  ...
  function renderStudents(studentsArray = students) {
    //after each time calling this function student table will be empty first
...
//Add student function
  function addStudent(name, age, grade, degree, email) {
    ...
    let filteredEqualObj = students.filter((student)=>{
         return student.name.toLowerCase()==newStudent.name.toLowerCase() && student.age==newStudent.age && student.email.toLowerCase()==newStudent.email.toLowerCase() && student.degree.toLowerCase()==newStudent.degree.toLowerCase();
    });
     
//if user already exist then popup shows otherwise push the userInfo into array    
    if(filteredEqualObj[0]===undefined){
